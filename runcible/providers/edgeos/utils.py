"""Helper functions for the EdgeOS BGP provider.

EdgeOS (Vyatta-derived) exposes its running configuration as a flat list of
``set ...`` commands via ``show configuration commands`` and applies changes
with ``set``/``delete`` statements under configure mode. This module holds the
pure (device-free) translation logic so it can be unit tested in isolation:

* ``parse_bgp_commands`` turns the ``show configuration commands`` output for
  ``protocols bgp`` into a vendor-neutral BGP state dict.
* the ``*_commands`` builders emit the ``set``/``delete protocols bgp <asn> ...``
  command tree for a single change (router-id, a neighbor, a network).

The vendor-neutral schema lives in ``runcible.modules.bgp`` (BGPResources /
BGPNeighborResources); these helpers map between it and EdgeOS CLI syntax.
"""
